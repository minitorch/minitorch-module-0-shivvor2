...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing.
    ...
    # Check "sigmoid of the negative"
    assert_close(float(1 - sigmoid(a)), sigmoid(-a))
    
    # Check value where function crosses x_axis
    if a == 0:
        assert_close(sigmoid(a), 0.5)

    # Check strictly increasing
    # For big values of a (>= 35), the decimal places of exp is saturated
    # Resulting into sigmoid(x_i), sigmoid(x_j) taking same values for different
    # x_i and x_j
    if abs(a) < 30:
        assert sigmoid(a) < sigmoid(float(a + 0.1))
